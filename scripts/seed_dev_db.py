...
за последние 14 дней, чтобы видеть дашборд (решения/TACoS/бюджеты) И новую
React-панель (список товаров, KPI, будущие графики задачи 3) с настоящими
строками, а не прочерками.
...
Наполняет:
  ...
  - decisions_log / tacos_daily — «сегодняшние» решения и TACoS одним днём,
    как было раньше — их читает старая Jinja-панель.
...
# «сегодняшние» цифры для decisions_log/tacos_daily (старая Jinja-панель), не
# трогаем их смысл, они были такими и до этой правки.
PRODUCTS = [
...
def main() -> None:
    ...
    try:
        ...
        # ---- 2) «Сегодня»: решения биддера + tacos_daily (старая Jinja-панель) ----
        for p in PRODUCTS:
            bid = p["bid"]
