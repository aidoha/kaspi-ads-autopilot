...
3. Учти: у правил есть действие `pause`, но у `marketing_client` пока нет эндпоинта
   паузы (только смена ставки) — паузу нужно либо добавить, либо маппить в минимальную ставку.
4. Выключи `dry_run` в `config/rules.yaml`, перезапусти сервис.
